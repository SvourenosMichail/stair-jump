...
function updatePurchasesData()
{
	...
	var queryData, queryJson = GPBilling_QueryPurchases(gpb_purchase_skutype_inapp);
	...
	if (queryJson != "")
	{
		...
		if (queryData.success)
		{
			...
			for (var i = 0; i < purchasesCount; i++)
			{
				...
				purchases[$ purchase.productId] = purchase;
			}
		}
	}
	
	// Now we do the same as the above but for purchases of type "subs" (subscriptions)
	var queryJson = GPBilling_QueryPurchases(gpb_purchase_skutype_subs);
	if (queryJson != "")
	{
		// Convert query json to data
		queryData = json_parse(queryJson);
